...
class Database:
    ...
    async def get_stats(self, user_id):
        async with self.pool.acquire() as conn:
            row = await conn.fetchrow("SELECT score, level FROM users WHERE user_id = $1", user_id)
            if row:
    ...
    async def set_birthdate(self, user_id, birth_date):
        async with self.pool.acquire() as conn:
            await conn.execute("UPDATE users SET birthdate = $1 WHERE user_id = $2", birth_date, user_id)

    async def get_birthdate(self, user_id):
        async with self.pool.acquire() as conn:
            return await conn.fetchval("SELECT birthdate FROM users WHERE user_id = $1", user_id)
    ...
    async def add_energy(self, user_id, amount=1):
        ...
        async with self.pool.acquire() as conn:
            current = await conn.fetchval("SELECT energy FROM users WHERE user_id = $1", user_id)
